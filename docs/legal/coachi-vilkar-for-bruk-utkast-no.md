# Vilkår for bruk av Coachi

**Sist oppdatert:** 10. mars 2026

Disse vilkårene gjelder for bruk av Coachi.

Ved å bruke appen godtar du vilkårene.

## 1. Om tjenesten
Coachi er en treningsapp som kan tilby:
- lydcoaching under økter
- treningshistorikk og oppsummeringer
- kontofunksjoner
- Premium-funksjoner
- stemme- og AI-baserte funksjoner når disse er tilgjengelige

Funksjonene kan endres over tid.

## 2. Konto og registrering
Noen deler av Coachi kan brukes uten konto. Andre funksjoner kan kreve innlogging.

Du er ansvarlig for at opplysningene du gir er riktige, og for å beskytte enheten og innloggingsinformasjonen din.

## 3. Abonnement og betaling
Coachi lanseres i gratis modus. Hvis betalte abonnement blir introdusert senere, vises pris, vilkar, fornyelse og oppsigelse tydelig i appen for brukeren før kjøp.

Hvis kjøp skjer via App Store, håndteres betaling og oppsigelse i henhold til Apples systemer og vilkår.

## 4. Tillatt bruk
Du kan bruke Coachi til personlig trening og coaching i samsvar med disse vilkårene.

Du skal ikke:
- bruke tjenesten i strid med loven
- forsøke å omgå sikkerhetsmekanismer eller tekniske begrensninger
- misbruke AI-, lyd- eller abonnementsfunksjoner
- kopiere eller distribuere innhold uten tillatelse
- bruke tjenesten på en måte som skader oss, andre brukere eller tredjepartsleverandører

## 5. Helseforbehold
Coachi er ikke medisinsk rådgivning og erstatter ikke helsepersonell.

Du er selv ansvarlig for å vurdere om trening passer for deg. Avbryt eller tilpass aktiviteten hvis du opplever smerte, svimmelhet, ubehag eller andre symptomer.

Hvis du er usikker på egen helse, bør du rådføre deg med helsepersonell før bruk.

## 6. Puls, sensorer og treningsdata
Coachi kan bruke puls og andre sensordata for å levere bedre coaching. Vi kan ikke garantere at slike data alltid er tilgjengelige eller korrekte.

Hvis puls mangler, kan appen fortsette med andre coachingmoduser. Du er fortsatt selv ansvarlig for innsats og sikker gjennomføring.

## 7. Talk to Coach og AI-funksjoner
Coachi kan tilby AI-baserte funksjoner, inkludert spørsmål og svar under eller etter trening.

Disse funksjonene er støtteverktøy. De kan være unøyaktige, ufullstendige eller utilgjengelige. De skal ikke brukes som medisinsk eller annen profesjonell rådgivning.

## 8. Tilgjengelighet
Vi forsøker å holde tjenesten tilgjengelig og oppdatert, men garanterer ikke feilfri eller uavbrutt drift.

Vi kan oppdatere, endre, begrense eller fjerne funksjoner når det er nødvendig av tekniske, produktmessige eller sikkerhetsmessige grunner.

## 9. Immaterielle rettigheter
Appen, navnet Coachi, lydfiler, design, teknologi og annet innhold tilhører Coachi eller våre lisensgivere.

Du får en begrenset, personlig og ikke-overførbar rett til å bruke appen i samsvar med disse vilkårene.

## 10. Ansvarsbegrensning
Så langt loven tillater det, er vi ikke ansvarlige for indirekte tap, driftstap, datatap eller tap som følge av feil bruk, sensorfeil, tjenesteavbrudd eller tredjepartstjenester.

Dette begrenser ikke ansvar som ikke lovlig kan fraskrives.

## 11. Oppsigelse og avslutning
Du kan når som helst slutte å bruke appen. Abonnement må sies opp i henhold til kjøpskanalens regler.

Vi kan suspendere eller avslutte tilgang ved mislighold, misbruk eller risiko for tjenesten eller andre brukere.

## 12. Personvern
Behandling av personopplysninger er regulert i personvernerklæringen.

## 13. Lovvalg og verneting
Vilkårene reguleres av norsk rett, med mindre annet følger av ufravikelig lov.

Tvister søkes løst i minnelighet først. Hvis det ikke lykkes, avgjøres saken av ordinære norske domstoler, med mindre annet følger av ufravikelig lov.

## 14. Kontakt
Spørsmål om vilkårene kan sendes til:

**Coachi**  
E-post: AI.Coachi@hotmail.com  
Nettside: https://coachi.app
