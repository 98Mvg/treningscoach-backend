...
struct WaveformView: View {
    ...
    var body: some View {
        TimelineView(.animation(minimumInterval: 1.0 / 20.0)) { timeline in
            ...
            HStack(spacing: 4) {
                ForEach(0..<barCount, id: \.self) { index in
                    ...
                    let normalizedWave = (wave + 1) / 2 // 0...1
                    ...
                    RoundedRectangle(cornerRadius: 3)
                        .fill(
                            LinearGradient(
                                colors: [
                                    CoachiTheme.primary.opacity(isActive ? 0.9 : 0.4),
                                    CoachiTheme.secondary.opacity(isActive ? 0.7 : 0.2)
                                ],
                                startPoint: .bottom,
                                endPoint: .top
                            )
        ...
        .onAppear {
            phases = (0..<barCount).map { index in
                Double(index) * 0.5 + Double.random(in: 0...1)
            }
        }
    }
}
...
#Preview {
    ZStack {
        ...
        VStack(spacing: 30) {
            ...
            WaveformView(isActive: true, height: 60)
        }
    }
}
