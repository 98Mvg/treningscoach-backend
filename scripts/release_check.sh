...
continuous_status="$(curl -s -o /dev/null -w "%{http_code}" -X POST "${BASE_URL}/coach/continuous")"
...
waitlist_json="$(curl -sf -X POST "${BASE_URL}/waitlist" \
...
welcome_json="$(curl -sf "${BASE_URL}/welcome?language=no&persona=personal_trainer")"
