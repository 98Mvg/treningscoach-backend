...
**TTS:** ElevenLabs `eleven_flash_v2_5` model. Qwen TTS disabled (too slow on CPU). Norwegian uses `language_code="nor"` (ISO 639-3). Danish uses `"dan"`.
...
2. Check `language_code="nor"` (ISO 639-3) is being passed to ElevenLabs `convert()` call — NOT `"nb"` which is unsupported
...
1. Check `voice_intelligence.py` signal quality threshold (currently `< 0.03`) — phone mics are noisy, don't over-silence
